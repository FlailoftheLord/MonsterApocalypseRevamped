...
#(October-07-2019)
  Decided to release the first half of the 0.12 update.
  New targettingsystem which runs through the world and only loaded chunks with active mobs in them.
  Also, block destroying is way more efficient and realistic. Block building is not avaliable yet (hence the first-half)
   Take note that there are alot of new and removed settings. You'll need to update all your settings files (delete and let them regen.)
   
   Please let me know how this works for y'all. I know i'm very slow on updating, but I want to make it as bug-free as possible. Thanks! 
